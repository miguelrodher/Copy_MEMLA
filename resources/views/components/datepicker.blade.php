...
this.{{$model}} = this.datePickerFormatDate(selectedDate);
...
return this.{{$model}} === this.datePickerFormatDate(d) ? true : false;
},
...
if ({{$model}}) {
currentDate = new Date(Date.parse({{$model}}));
}
datePickerMonth = currentDate.getMonth();
datePickerYear = currentDate.getFullYear();
datePickerDay = currentDate.getDay();
{{$model}} = datePickerFormatDate( currentDate );
datePickerCalculateDays();
" x-cloak>
    <div class="container px-1 py-2 mx-auto md:py-2">
        <div class="w-full mb-2">
            <label {{$reactive}}for="{{$id}}" class="block mb-1 text-sm font-medium text-neutral-500">{{$label}}</label>
            <div class="relative w-[17rem]">
                <input x-ref="datePickerInput" {{$reactive}}id="{{$id}}" type="text" name="{{$name}}" @click="datePickerOpen=!datePickerOpen" x-model="{{$model}}" x-on:keydown.escape="datePickerOpen=false"
                    class="{{$class}} flex w-full h-10 px-3 py-2 text-sm bg-white border rounded-md text-neutral-600 border-neutral-300 ring-offset-background placeholder:text-neutral-400 focus:border-neutral-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-neutral-400 disabled:cursor-not-allowed disabled:opacity-50"
 ...
 <template x-for="(day, dayIndex) in datePickerDaysInMonth" :key="dayIndex">
     <div class="px-0.5 mb-1 aspect-square">
         <div
             ...
             :class="{
'bg-memla-400': datePickerIsToday(day) == true,
'text-gray-600 hover:bg-memla-200': datePickerIsToday(day) == false && datePickerIsSelectedDate(day) == false,
'bg-memla-800 text-white hover:bg-opacity-75': datePickerIsSelectedDate(day) == true
}"
